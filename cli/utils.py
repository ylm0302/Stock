...
TICKER_INPUT_EXAMPLES = "Examples: SPY, CNC.TO, 7203.T, 0700.HK, 110022(基金)"
...
# 基金模式分析师标签（与 ANALYST_ORDER 一一对应，wire value 相同）
FUND_ANALYST_ORDER = [
    ("Fund Market Analyst  (净值走势分析)", AnalystType.MARKET),
    ("Fund Sentiment Analyst (情绪与排名分析)", AnalystType.SOCIAL),
    ("Fund News Analyst  (宏观与新闻分析)", AnalystType.NEWS),
    ("Fund Fundamentals Analyst (持仓/费率/基金经理)", AnalystType.FUNDAMENTALS),
]
...
# 6位纯数字 → 国内公募基金代码
import re as _re
_FUND_CODE_RE = _re.compile(r"^\d{6}$")
...
def detect_asset_type(ticker: str) -> AssetType:
    ...
    # 6位纯数字 → 国内公募基金
    if _FUND_CODE_RE.match(ticker.strip()):
...
def select_analysts(asset_type: AssetType = AssetType.STOCK) -> List[AnalystType]:
    ...
    if asset_type == AssetType.FUND:
        order = FUND_ANALYST_ORDER
        instruction_extra = "\n  (基金模式：所有分析师均使用基金专属工具)"
    else:
        available = filter_analysts_for_asset_type(
            [value for _, value in ANALYST_ORDER], asset_type
        )
        order = [(d, v) for d, v in ANALYST_ORDER if v in available]
        instruction_extra = ""
