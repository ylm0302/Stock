...
class MessageBuffer:
    ...
    # 基金模式分析师名称映射（wire value 相同，显示名不同）
    FUND_ANALYST_MAPPING = {
        "market": "Market Analyst",       # 内部 agent 节点名不变
        "social": "Sentiment Analyst",
        "news": "News Analyst",
        "fundamentals": "Fundamentals Analyst",
    }
...
def get_user_selections():
    ...
    console.print(
        create_question_box(
            "Step 1: Ticker / Fund Code",
            "Enter the exact ticker symbol or fund code to analyze.\n"
            "  • Stock examples: SPY, NVDA, 600519.SS, 0700.HK\n"
            "  • Crypto examples: BTC-USD, ETH-USDT\n"
            "  • 🇨🇳 Fund  examples: 110022 (易方达消费), 000961 (天弘沪深300), 161725 (招商白酒)\n"
            "    (6-digit code → auto-detected as fund mode)",
            "SPY",
        )
    )
    ...
    if asset_type == AssetType.FUND:
        console.print(
            f"[bold green]✅ Detected asset type:[/bold green] [bold yellow]Fund (基金模式)[/bold yellow] "
            f"— will use fund-specific analysts & data sources"
        )
    else:
        console.print(
...
def run_analysis(checkpoint: bool = False):
    ...
    # 基金模式：重编译图以使用基金专属分析师
    asset_type_str = selections["asset_type"]
    if asset_type_str == "fund":
        graph.tool_nodes = graph._fund_tool_nodes
        graph.graph_setup.tool_nodes = graph._fund_tool_nodes
        graph.workflow = graph.graph_setup.setup_graph(selected_analyst_keys, asset_type="fund")
